eagle.define('eagle_dashboard.eagle_dashboard', function (require) {
    ...
    var KsDashboardNinja = AbstractAction.extend(ControlPanelMixin, {
        ...
        eagle_set_update_interval : function(){
            ...
            if (self.eagle_dashboard_data.eagle_item_data){
                ...
                Object.keys(self.eagle_dashboard_data.eagle_item_data).forEach(function (item_id){
                    ...
                    var updateValue = item_data["eagle_update_items_data"];
                    if (updateValue){
                        if (!(item_id in self.ksUpdateDashboard)){
                             if(['eagle_tile','eagle_list_view','eagle_kpi'].indexOf(item_data['eagle_dashboard_item_type'])>=0){
        ...
        events: {
            ...
            'click .eagle_load_previous': 'ksLoadPreviousRecords',
            'click .eagle_load_next': 'ksLoadMoreRecords',
            ...
            'change .eagle_input_import_item_button': 'ksImportItem',
            ...
            'click .eagle_chart_json_export': 'ksItemExportJson',
            ...
            'click .eagle_list_canvas_click': 'onChartCanvasClick',
        ...
        ksRenderChartColorOptions: function (e) {
            ...
            if (!$(e.currentTarget).parent().hasClass('eagle_date_filter_selected')) {
                ...
                var chart_data = JSON.parse(item_data.eagle_chart_data);
                this.ksChartColors(e.currentTarget.dataset.chartColor, this.chart_container[$parent.data().itemId], $parent.data().chartType, $parent.data().chartFamily,item_data.eagle_bar_chart_stacked,item_data.eagle_semi_circle_chart,item_data.eagle_show_data_value,chart_data,item_data)
                this._rpc({
                    ...
                    args: [$parent.data().itemId, {
                       ...
                       self.eagle_dashboard_data.eagle_item_data[$parent.data().itemId]['eagle_chart_item_color'] = e.currentTarget.dataset.chartColor;
                });
            }
        },
        ...
        onKsDashboardMenuContainerShow: function (e) {
            ...
            var item_id = $(e.currentTarget).find('.eagle_chart_inner_buttons.eagle_dashboard_more_action').data('itemId');
        ...
        onKsDashboardMenuContainerHide: function(e) {
            ...
            var updateValue = this.eagle_dashboard_data.eagle_item_data[item_id]["eagle_update_items_data"];
            if (updateValue) {
            ...
            if (this.eagle_dashboard_data.eagle_item_data[item_id]['isDrill'] == true) {
                clearInterval(this.ksUpdateDashboard[item_id]);
            }
        },
...
//            ".eagle_date_input_fields"
        ...
        _ksRenderDashboardTile: function (tile) {
            var self = this;
            var eagle_container_class = 'grid-stack-item';
            var eagle_inner_container_class = 'grid-stack-item-content';
            ...
            var count = field_utils.format.float(tile.eagle_record_count,Float64Array);
        ...
        _renderGraph: function (item) {
            ...
            if (chart_id in self.gridstackConfig) {
                self.grid.addWidget($eagle_gridstack_container, self.gridstackConfig[chart_id].x, self.gridstackConfig[chart_id].y, self.gridstackConfig[chart_id].width, self.gridstackConfig[chart_id].height, false, 11, null, 3, null, chart_id);
            } else {
                self.grid.addWidget($eagle_gridstack_container, 0, 0, 13, 4, true, 11, null, 3, null, chart_id);
            }
            self._renderChart($eagle_gridstack_container,item);
        },

        _renderChart($eagle_gridstack_container,item){
            var self = this;
            var chart_data = JSON.parse(item.eagle_chart_data);
            var isDrill = item.isDrill ? item.isDrill : false;
            var chart_id = item.id,
            ...
            $eagle_gridstack_container.find('.eagle_color_pallate').data({chartType:chart_type,chartFamily:chart_family});
            {chartType:"pie"}
            ...
            if (!item.eagle_show_records) {
                $eagle_gridstack_container.find('.eagle_dashboard_item_chart_info').hide();
            }
                    ...
                    },
                    {
                        ...
                        ticks: {
                            ...
                            callback : function(value, index, values){
                                var eagle_selection = chart_data.eagle_selection;
                                if (eagle_selection === 'monetary'){
                                    var eagle_currency_id = chart_data.eagle_currency;
                                    var eagle_data = KsGlobalFunction.ksNumFormatter(value,1);
                                    eagle_data = KsGlobalFunction.eagle_monetary(eagle_data, eagle_currency_id);
                                    return eagle_data;
                                }
                                else if (eagle_selection === 'custom'){
                                    var eagle_field = chart_data.eagle_field;
                                    return KsGlobalFunction.ksNumFormatter(value,1) +' '+ eagle_field;
                                }
                                else {
            ...
            if(chart_data && chart_data["datasets"].length>0) self.ksChartColors(item.eagle_chart_item_color, ksMyChart, chart_type, chart_family,item.eagle_bar_chart_stacked,item.eagle_semi_circle_chart,item.eagle_show_data_value,chart_data,item);
        ...
        ksChartColors: function (palette, ksMyChart, ksChartType, ksChartFamily,stack, semi_circle,eagle_show_data_value,chart_data,item) {
            chart_data;
            var self = this;
            ...
            if (ksChartFamily == "circle") {
                ...
                options.tooltips.callbacks = {
                                              title: function(tooltipItem, data) {
                                                    ...
                                                    if (eagle_selection === 'monetary'){
                                                        var eagle_currency_id = chart_data.eagle_currency;
                                                        k_amount = KsGlobalFunction.eagle_monetary(k_amount, eagle_currency_id);
                                                        return data.datasets[tooltipItem[0].datasetIndex]['label']+" : " + k_amount
                                                    }
                                                    else if (eagle_selection === 'custom'){
                                                        var eagle_field = chart_data.eagle_field;
//                                                        eagle_type = field_utils.format.char(eagle_field);
                                                        k_amount = field_utils.format.float(k_amount,Float64Array);
                                                        return data.datasets[tooltipItem[0].datasetIndex]['label']+" : " + k_amount+" "+ eagle_field;
                                                    }
                                                    else {
                                                        k_amount = field_utils.format.float(k_amount,Float64Array);
                                                        return data.datasets[tooltipItem[0].datasetIndex]['label']+" : " + k_amount
                ...
                options.tooltips.callbacks = {
                    label: function(tooltipItem, data) {
                        ...
                        if (eagle_selection === 'monetary'){
                           ...
                           // eagle_type = field_utils.format.char(eagle_field);
                            k_amount = field_utils.format.float(k_amount,Float64Array);
                            return data.datasets[tooltipItem.datasetIndex]['label']+" : " + k_amount+" "+ eagle_field;
                        }
                        else {
                            k_amount = field_utils.format.float(k_amount,Float64Array);
                            return data.datasets[tooltipItem.datasetIndex]['label']+" : " + k_amount
        ...
        onChartCanvasClick: function(evt) {
            var self = this;
            if (evt.currentTarget.classList.value !== 'eagle_list_canvas_click') {
                        ...
                        if (item_data.max_sequnce != 0 && sequnce < item_data.max_sequnce) {
                            self._rpc({
                                ...
                                if (result.eagle_chart_data) {
                                    ...
                                    if (self.eagle_dashboard_data.eagle_item_data[item_id].domains) {
                                    ...
                                    $(self.$el.find(".grid-stack-item[data-gs-id=" + item_id + "]").children()[0]).find(".eagle_dashboard_item_drill_up").removeClass('d-none');
                                    $(self.$el.find(".grid-stack-item[data-gs-id=" + item_id + "]").children()[0]).find(".eagle_dashboard_item_chart_info").removeClass('d-none')
                                    $(self.$el.find(".grid-stack-item[data-gs-id=" + item_id + "]").children()[0]).find(".eagle_dashboard_color_option").removeClass('d-none')
                                    $(self.$el.find(".grid-stack-item[data-gs-id=" + item_id + "]").children()[0]).find(".eagle_dashboard_quick_edit_action_popup").removeClass('d-sm-block ');
                                    $(self.$el.find(".grid-stack-item[data-gs-id=" + item_id + "]").children()[0]).find(".eagle_dashboard_more_action").addClass('d-none');
                                    ...
                                    if ('domains' in self.eagle_dashboard_data.eagle_item_data[item_id]) {
                                        self.eagle_dashboard_data.eagle_item_data[item_id]['domains'][result.sequence] = JSON.parse(result.eagle_list_view_data).previous_domain;
                                    } else {
                                        self.eagle_dashboard_data.eagle_item_data[item_id]['domains'] = {}
                                        self.eagle_dashboard_data.eagle_item_data[item_id]['domains'][result.sequence] = JSON.parse(result.eagle_list_view_data).previous_domain;
                                    }
                                    self.eagle_dashboard_data.eagle_item_data[item_id]['isDrill'] = true;
                                    self.eagle_dashboard_data.eagle_item_data[item_id]['sequnce'] = result.sequence;
                                    self.eagle_dashboard_data.eagle_item_data[item_id]['eagle_list_view_data'] = result.eagle_list_view_data;
                                    self.eagle_dashboard_data.eagle_item_data[item_id]['eagle_list_view_type'] = result.eagle_list_view_type;
                                    self.eagle_dashboard_data.eagle_item_data[item_id]['eagle_dashboard_item_type'] = 'eagle_list_view';

                                    $(self.$el.find(".grid-stack-item[data-gs-id=" + item_id + "]").children()[0]).find(".eagle_dashboard_item_drill_up").removeClass('d-none');

                                    $(self.$el.find(".grid-stack-item[data-gs-id=" + item_id + "]").children()[0]).find(".eagle_dashboard_item_chart_info").addClass('d-none')
                                    $(self.$el.find(".grid-stack-item[data-gs-id=" + item_id + "]").children()[0]).find(".eagle_dashboard_color_option").addClass('d-none')
                                    $(self.$el.find(".grid-stack-item[data-gs-id=" + item_id + "]").children()[0]).find(".card-body").empty();
                                    $(self.$el.find(".grid-stack-item[data-gs-id=" + item_id + "]").children()[0]).find(".eagle_dashboard_quick_edit_action_popup").removeClass('d-sm-block ');
                                    ...
                                    $(self.$el.find(".grid-stack-item[data-gs-id=" + item_id + "]").children()[0]).find(".card-body").append($container).addClass('eagle_overflow');
                                }
                            });
                        } else {
                            if (item_data.action) {
                            ...
                            if (item_data.eagle_show_records) {
                ...
                var item_data = self.eagle_dashboard_data.eagle_item_data[item_id]
                if (self.eagle_dashboard_data.eagle_item_data[item_id].max_sequnce) {
                    ...
                    if ($(evt.target).parent().data().last_seq !== sequence) {
                        self._rpc({
                            ...
                            if (result.eagle_list_view_data) {
                                if (self.eagle_dashboard_data.eagle_item_data[item_id].domains) {
                                    self.eagle_dashboard_data.eagle_item_data[item_id]['domains'][result.sequence] = JSON.parse(result.eagle_list_view_data).previous_domain;
                                } else {
                                    self.eagle_dashboard_data.eagle_item_data[item_id]['domains'] = {}
                                ...
                                $(self.$el.find(".grid-stack-item[data-gs-id=" + item_id + "]").children()[0]).find(".card-body").empty();
                                $(self.$el.find(".grid-stack-item[data-gs-id=" + item_id + "]").children()[0]).find(".eagle_dashboard_item_drill_up").removeClass('d-none');
                                $(self.$el.find(".grid-stack-item[data-gs-id=" + item_id + "]").children()[0]).find(".eagle_dashboard_item_action_export").addClass('d-none');
                                $(self.$el.find(".grid-stack-item[data-gs-id=" + item_id + "]").children()[0]).find(".eagle_dashboard_quick_edit_action_popup").removeClass('d-sm-block ');
                ...
                if ('domains' in item_data) {
                    ...
                    if (sequnce >= 0) {
                        self._rpc({
                            ...
                            if (result.eagle_chart_type)  self.eagle_dashboard_data.eagle_item_data[item_id]['eagle_dashboard_item_type'] = result.eagle_chart_type;
                            $(self.$el.find(".grid-stack-item[data-gs-id=" + item_id + "]").children()[0]).find(".eagle_dashboard_item_drill_up").removeClass('d-none');
                            $(self.$el.find(".grid-stack-item[data-gs-id=" + item_id + "]").children()[0]).find(".card-body").empty();
                            if (result.eagle_chart_data) {
                                var item_data = self.eagle_dashboard_data.eagle_item_data[item_id];
                                ...
                                var item_data = self.eagle_dashboard_data.eagle_item_data[item_id]
                                var $container = self.renderListViewData(item_data);
                                $(self.$el.find(".grid-stack-item[data-gs-id=" + item_id + "]").children()[0]).find(".eagle_pager").addClass('d-none');
                                $(self.$el.find(".grid-stack-item[data-gs-id=" + item_id + "]").children()[0]).find(".eagle_dashboard_item_chart_info").addClass('d-none')
                                $(self.$el.find(".grid-stack-item[data-gs-id=" + item_id + "]").children()[0]).find(".eagle_dashboard_color_option").addClass('d-none')
                                $(self.$el.find(".grid-stack-item[data-gs-id=" + item_id + "]").children()[0]).find(".card-body").append($container);
                            }
                        ...
                        $(self.$el.find(".grid-stack-item[data-gs-id=" + item_id + "]").children()[0]).find(".eagle_dashboard_item_drill_up").addClass('d-none');
                        $(self.$el.find(".grid-stack-item[data-gs-id=" + item_id + "]").children()[0]).find(".eagle_dashboard_item_chart_info").removeClass('d-none')
                        $(self.$el.find(".grid-stack-item[data-gs-id=" + item_id + "]").children()[0]).find(".eagle_dashboard_color_option").removeClass('d-none')
                        $(self.$el.find(".grid-stack-item[data-gs-id=" + item_id + "]").children()[0]).find(".eagle_dashboard_quick_edit_action_popup").addClass('d-sm-block ');
                        $(self.$el.find(".grid-stack-item[data-gs-id=" + item_id + "]").children()[0]).find(".eagle_dashboard_more_action").removeClass('d-none');
                        $(self.$el.find(".grid-stack-item[data-gs-id=" + item_id + "]").children()[0]).find(".eagle_dashboard_item_action_export").removeClass('d-none')
                    ...
                    if(!domain) $(self.$el.find(".grid-stack-item[data-gs-id=" + item_id + "]").children()[0]).find(".eagle_dashboard_item_drill_up").addClass('d-none');
        ...
        ksFetchChartItem: function(id) {
            ...
            return self._rpc({
                ...
                method: 'eagle_fetch_item',
                args: [
                    [item_data.id], self.eagle_dashboard_id
                ],
                context: self.getContext(),
            }).then(function(new_item_data) {
                this.eagle_dashboard_data.eagle_item_data[id] = new_item_data[id];
                $(self.$el.find(".grid-stack-item[data-gs-id=" + id + "]").children()[0]).find(".card-body").empty();
                var item_data = self.eagle_dashboard_data.eagle_item_data[id]
                if (item_data.eagle_list_view_data) {
                    var item_view = $(self.$el.find(".grid-stack-item[data-gs-id=" + id + "]").children()[0]);
                    var $container = self.renderListViewData(item_data);
                    item_view.find(".card-body").append($container);
                    var eagle_length = JSON.parse(item_data['eagle_list_view_data']).data_rows.length
                    if (new_item_data["eagle_list_view_type"] === "ungrouped" && JSON.parse(item_data['eagle_list_view_data']).data_rows.length) {
                        item_view.find('.eagle_pager').removeClass('d-none');
                        if (eagle_length < 15) item_view.find('.eagle_load_next').addClass('eagle_event_offer_list');
                        item_view.find('.eagle_value').text("1-" + JSON.parse(item_data['eagle_list_view_data']).data_rows.length);
                    } else {
                        item_view.find('.eagle_pager').addClass('d-none');
                    }
                } else {
        ...
        onChartMoreInfoClick : function(evt){
            ...
            var item_data = self.eagle_dashboard_data.eagle_item_data[item_id];
            var groupBy = item_data.eagle_chart_groupby_type === 'relational_type' || item_data.eagle_chart_groupby_type === 'other'
                    ? item_data.eagle_chart_relation_groupby_name : item_data.eagle_chart_relation_groupby_name + ':' + item_data.eagle_chart_date_groupby;
            var subgroupBy =  item_data.eagle_chart_sub_groupby_type === 'relational_type' || item_data.eagle_chart_sub_groupby_type === 'other'
                    ? item_data.eagle_chart_relation_sub_groupby_name : item_data.eagle_chart_relation_sub_groupby_name + ':' + item_data.eagle_chart_date_sub_groupby;
            var domain = JSON.parse(item_data.eagle_chart_data).previous_domain
            if (groupBy){
                eagle_group_by.push(groupBy);
            }
            if (subgroupBy){
                eagle_group_by.push(subgroupBy);
            }
            if (item_data.eagle_show_records) {
                if (item_data.action) {
                    var action = Object.assign({}, item_data.action);
        ...
        _ksRenderEditMode: function () {
            ...
            self.$el.find('.eagle_start_tv_dashboard').addClass('eagle_hide');
...
//            self.$el.find('.eagle_list_view_container').addClass('eagle_item_not_click');
        ...
        _ksRenderActiveMode: function () {
            ...
            self.$el.find('.eagle_start_tv_dashboard').removeClass('eagle_hide');
        ...
        onAddItemTypeClick: function(e) {
            var self = this;
            if (e.currentTarget.dataset.item !== "eagle_json") {
                self.do_action({
                    type: 'ir.actions.act_window',
        ...
        ksImportItemJson: function(e) {
            var self = this;
            $('.eagle_input_import_item_button').click();
        },
        ...
        ksImportItem: function(e) {
            ...
            fileReader.onload = function() {
                $('.eagle_input_import_item_button').val('');
                framework.blockUI();
                self._rpc({
                    model: 'eagle_dashboard.board',
                    method: 'eagle_import_item',
                    args: [self.eagle_dashboard_id],
                    kwargs: {
                        file: fileReader.result,
                        dashboard_id: self.eagle_dashboard_id
                    }
                }).then(function(result) {
                    if (result === "Success") {
            ...
            fileReader.readAsText($('.eagle_input_import_item_button').prop('files')[0]);
        },
        ...
        ksUpdateDashboardItem : function(ids) {
            ...
            for (var i = 0; i < ids.length; i++) {
                ...
                if (item_data['eagle_dashboard_item_type'] == 'eagle_list_view') {
                    ...
                    var eagle_length = JSON.parse(item_data['eagle_list_view_data']).data_rows.length;
                    if (item_data['eagle_list_view_type'] === 'ungrouped' && JSON.parse(item_data['eagle_list_view_data']).data_rows.length) {
                        if (item_view.find('.eagle_pager_name')) {
                            item_view.find('.eagle_pager_name').empty();
                            var $eagle_pager_container = QWeb.render('eagle_pager_template', {
                                item_id: ids[i],
                                intial_count: 20,
                                offset : 1
                            })
                            item_view.find('.eagle_pager_name').append($($eagle_pager_container));
                        }
                        if (eagle_length < 20) item_view.find('.eagle_load_next').addClass('eagle_event_offer_list');
                        item_view.find('.eagle_value').text("1-" + JSON.parse(item_data['eagle_list_view_data']).data_rows.length);
                    } else {
                        item_view.find('.eagle_pager').addClass('d-none');
                    }
                } else if (item_data['eagle_dashboard_item_type'] == 'eagle_tile') {
                    var item_view = self._ksRenderDashboardTile(item_data);
                    self.$el.find(".grid-stack-item[data-gs-id=" + item_data.id + "]").empty();
                    self.$el.find(".grid-stack-item[data-gs-id=" + item_data.id + "]").append($(item_view).find('.eagle_dashboarditem_id'));
                } else if (item_data['eagle_dashboard_item_type'] == 'eagle_kpi') {
        ...
        _ksSaveCurrentLayout: function () {
            ...
            if (self.eagle_dashboard_data.eagle_gridstack_config) {
                _.extend(grid_config, JSON.parse(self.eagle_dashboard_data.eagle_gridstack_config))
            }
        ...
        _renderListView: function(item, grid) {
            ...
            var list_view_data = JSON.parse(item.eagle_list_view_data),
                pager = item.eagle_list_view_type === "ungrouped" ? true : false,
                item_id = item.id,
            ...
            if (length < 15) {
                $eagle_gridstack_container.find('.eagle_load_next').addClass('eagle_event_offer_list');
            }
            if(length == 0) {
                $eagle_gridstack_container.find('.eagle_pager').addClass('d-none');
            }
            $eagle_gridstack_container.find('.card-body').append($ksItemContainer);
            if (pager){
                $eagle_gridstack_container.find('.eagle_list_canvas_click').removeClass('eagle_list_canvas_click');
            }

            item.$el = $eagle_gridstack_container;
            if (item_id in self.gridstackConfig) {
                grid.addWidget($eagle_gridstack_container, self.gridstackConfig[item_id].x, self.gridstackConfig[item_id].y, self.gridstackConfig[item_id].width, self.gridstackConfig[item_id].height, false, 11, null, 3, null, item_id);
            } else {
                grid.addWidget($eagle_gridstack_container, 0, 0, 13, 4, true, 12, null, 3, null, item_id);
            }
        },
        ...
        renderListViewData: function(item) {
            ...
            var $ksItemContainer = $(QWeb.render('eagle_list_view_table', {
                ...
                item_id: item_id,
                list_type: item.eagle_list_view_type,
            }));
            this.list_container = $ksItemContainer;

            if (item.eagle_list_view_type === "ungrouped") {
                $ksItemContainer.find('.eagle_list_canvas_click').removeClass('eagle_list_canvas_click');
            }

            if (!item.eagle_show_records) {
                $ksItemContainer.find('#eagle_item_info').hide();
            }
        ...
        ksItemExportJson: function(e) {
            var itemId = $(e.target).parents('.eagle_dashboard_item_button_container')[0].dataset.item_id;
            var name = this.eagle_dashboard_data.eagle_item_data[itemId].name;
            var data = {
                'header': name,
                item_id: itemId,
            }
            framework.blockUI();
            this.getSession().get_file({
                url: '/eagle_dashboard/export/item_json',
                data: {
        ...
        ksLoadMoreRecords: function(e) {
            var self = this;
            var eagle_intial_count = e.target.parentElement.dataset.prevOffset;
            var eagle_offset = e.target.parentElement.dataset.next_offset;
            ...
            this._rpc({
                model: 'eagle_dashboard.board',
                method: 'eagle_get_list_view_data_offset',
                context: self.getContext(),
                args: [parseInt(itemId), {
                    eagle_intial_count: eagle_intial_count,
                    offset: eagle_offset,
                    }, parseInt(self.eagle_dashboard_id)],
            }).then(function(result) {
                var item_data = self.eagle_dashboard_data.eagle_item_data[itemId];
                self.eagle_dashboard_data.eagle_item_data[itemId]['eagle_list_view_data'] = result.eagle_list_view_data;
                var item_view = self.$el.find(".grid-stack-item[data-gs-id=" + item_data.id + "]");
                item_view.find('.card-body').empty();
                item_view.find('.card-body').append(self.renderListViewData(item_data));
                $(e.currentTarget).parents('.eagle_pager').find('.eagle_value').text(result.offset + "-" + result.next_offset);
                e.target.parentElement.dataset.next_offset = result.next_offset;
                e.target.parentElement.dataset.prevOffset = result.offset;
                $(e.currentTarget.parentElement).find('.eagle_load_previous').removeClass('eagle_event_offer_list');
                if (result.next_offset < parseInt(result.offset) + 14 || result.next_offset == item_data.eagle_record_count || result.next_offset === result.limit){
                    $(e.currentTarget).addClass('eagle_event_offer_list');
                }
            });
        },
        ...
        ksLoadPreviousRecords: function(e) {
            var self = this;
            var eagle_offset = parseInt(e.target.parentElement.dataset.prevOffset) - 16;
            var eagle_intial_count = e.target.parentElement.dataset.next_offset;
            var itemId = e.currentTarget.dataset.itemId;
            if (eagle_offset <= 0) {
                var updateValue = self.eagle_dashboard_data.eagle_item_data[itemId]["eagle_update_items_data"];
                if (updateValue) {
                    var updateinterval = setInterval(function() {
            ...
            this._rpc({
                ...
                e.target.parentElement.dataset.next_offset = result.next_offset;
                e.target.parentElement.dataset.prevOffset = result.offset;
                $(e.currentTarget.parentElement).find('.eagle_load_next').removeClass('eagle_event_offer_list');
                if (result.offset === 1) {
                    $(e.currentTarget).addClass('eagle_event_offer_list');
                }
            });
